      ...
      {wxMode === "auto" && (
        <>
          <p style={{ margin: "0 0 10px", fontSize: 12, color: "#9aa8b5" }}>
            Generates realistic Dallas temps based on the season. No API needed — instant and always works.
          </p>
        ...
        <>
          {!manRows.length ? (
            <>
              ...
              <Btn onClick={setupRows} disabled={!startD} color={BLUE}>
                Set Up Fields
              </Btn>
            </>
          ) : (
            <>
                    ...
                    {manRows.map((r, i) => (
                      <tr key={i} style={{ borderBottom: "1px solid #f0f0f0" }}>
                        ...
                        <td style={{ padding: "4px" }}>
                          <label htmlFor={`wx-cond-${i}`} style={{ display: "none" }}>
                            Condition
                          </label>
                          <select
                            id={`wx-cond-${i}`}
                            value={r.cond}
                            onChange={(e) => updateRow(i, "cond", e.target.value)}
                            style={{
                              fontSize: 11,
                              padding: "4px",
                        ...
                        {[
                          { f: "hi", label: "High", placeholder: "73" },
                          { f: "lo", label: "Low", placeholder: "47" },
                          { f: "rain", label: "Rain %", placeholder: "2%" },
                        ].map(({ f, label, placeholder }) => (
                          <td key={f} style={{ padding: "4px 3px" }}>
                            <label htmlFor={`wx-${f}-${i}`} style={{ display: "none" }}>
                              {label}
                            </label>
                            <input
                              id={`wx-${f}-${i}`}
              ...
              <Btn onClick={applyManual} color={BLUE} full>
                Apply Forecast
              </Btn>
            </>
          )}
        ...
        <div style={{ marginTop: 12 }}>
          <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
            {forecast.map((d, i) => (
              ...
              >
                <div style={{ fontSize: 7, fontWeight: "bold", color: "#9aa8b5", textTransform: "uppercase" }}>
                  {d.day}
                </div>
